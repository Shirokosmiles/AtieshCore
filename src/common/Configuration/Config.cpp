...
T ConfigMgr::GetValueDefault(std::string const& name, T def, bool quiet) const
{
    ...
    catch (bpt::ptree_bad_path const&)
    {
        ...
        if (envVar)
        {
            ...
            if (!castedVar)
            {
                FMT_LOG_ERROR("server.loading", "Bad value defined for name {} in environment variables, going to use default instead", name);
                return def;
            }

            if (!quiet)
                FMT_LOG_WARN("server.loading", "Missing name {} in config file {}, recovered with environment '{}' value.", name, _filename, envVar->c_str());

            return *castedVar;
        }
        else if (!quiet)
        {
            FMT_LOG_WARN("server.loading", "Missing name {} in config file {}, add \"{} = {}\" to this file",
                name, _filename, name, std::to_string(def));
        }
    }
    catch (bpt::ptree_bad_data const&)
    {
        FMT_LOG_ERROR("server.loading", "Bad value defined for name {} in config file {}, going to use {} instead",
            name, _filename, std::to_string(def));
    }
