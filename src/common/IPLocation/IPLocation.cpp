...
void IpLocationStore::Load()
{
    ...
    FMT_LOG_INFO("server.loading", "Loading IP Location Database...");
    ...
    if (!databaseFile)
    {
        FMT_LOG_ERROR("server.loading", "IPLocation: No ip database file exists ({}).", databaseFilePath);
        return;
    }

    if (!databaseFile.is_open())
    {
        FMT_LOG_ERROR("server.loading", "IPLocation: Ip database file ({}) can not be opened.", databaseFilePath);
        return;
    }
    ...
    FMT_LOG_INFO("server.loading", ">> Loaded {} ip location entries.", _ipLocationStore.size());
}
