...
void AppenderConsole::InitColors(std::string const& name, std::string_view str)
{
    ...
    {
        throw InvalidAppenderArgsException(Trinity::StringFormat("Log::CreateAppenderFromConfig: Invalid color data '{}' for console appender {} (expected {} entries, got %zu)",
            std::string(str), name, NUM_ENABLED_LOG_LEVELS, colorStrs.size()));
    }
    ...
    for (uint8 i = 0; i < NUM_ENABLED_LOG_LEVELS; ++i)
    {
        ...
        else
        {
            throw InvalidAppenderArgsException(Trinity::StringFormat("Log::CreateAppenderFromConfig: Invalid color '{}' for log level {} on console appender {}",
                std::string(colorStrs[i]), EnumUtils::ToTitle(static_cast<LogLevel>(i)), name));
        }
    }
