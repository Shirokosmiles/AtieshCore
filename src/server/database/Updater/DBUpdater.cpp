...
bool DBUpdaterUtil::CheckExecutable()
{
    ...
    if (!is_regular_file(exe))
    {
        ...
        FMT_LOG_FATAL("sql.updates", "Didn't find any executable MySQL binary at \'{}\' or in path, correct the path in the *.conf (\"MySQLExecutable\").",
...
{
    FMT_LOG_INFO("sql.updates", "Database \"{}\" does not exist, do you want to create it? [yes (default) / no]: ",
    ...
    FMT_LOG_INFO("sql.updates", "Creating database \"{}\"...", pool.GetConnectionInfo()->database);
    ...
    if (!file.is_open())
    {
        FMT_LOG_FATAL("sql.updates", "Failed to create temporary query file \"{}\"!", temp.generic_string());
        return false;
    }
    ...
    {
        FMT_LOG_FATAL("sql.updates", "Failed to create database {}! Does the user (named in *.conf) have `CREATE`, `ALTER`, `DROP`, `INSERT` and `DELETE` privileges on the MySQL server?", pool.GetConnectionInfo()->database);
        boost::filesystem::remove(temp);
        return false;
    }

    FMT_LOG_INFO("sql.updates", "Done.");
    boost::filesystem::remove(temp);
    return true;
}
...
bool DBUpdater<T>::Update(DatabaseWorkerPool<T>& pool)
{
    ...
    FMT_LOG_INFO("sql.updates", "Updating {} database...", DBUpdater<T>::GetTableName());

    Path const sourceDirectory(BuiltInConfig::GetSourceDirectory());

    if (!is_directory(sourceDirectory))
    {
        FMT_LOG_ERROR("sql.updates", "DBUpdater: The given source directory {} does not exist, change the path to the directory where your sql directory exists (for example c:\\source\\trinitycore). Shutting down.", sourceDirectory.generic_string());
        return false;
    }
    ...
    if (!result.updated)
        FMT_LOG_INFO("sql.updates", ">> {} database is up-to-date! {}", DBUpdater<T>::GetTableName(), info);
    else
        FMT_LOG_INFO("sql.updates", ">> Applied {} {}. {}", result.updated, result.updated == 1 ? "query" : "queries", info);
...
bool DBUpdater<T>::Populate(DatabaseWorkerPool<T>& pool)
{
    ...
    FMT_LOG_INFO("sql.updates", "Database {} is empty, auto populating it...", DBUpdater<T>::GetTableName());

    std::string const p = DBUpdater<T>::GetBaseFile();
    if (p.empty())
    {
        FMT_LOG_INFO("sql.updates", ">> No base file provided, skipped!");
        return true;
    }
    ...
    if (!exists(base))
    {
        switch (DBUpdater<T>::GetBaseLocationType())
        {
            ...
            {
                FMT_LOG_ERROR("sql.updates", ">> Base file \"{}\" is missing. Try fixing it by cloning the source again.",
                    base.generic_string());
            ...
            case LOCATION_DOWNLOAD:
            {
                ...
                FMT_LOG_ERROR("sql.updates", ">> File \"{}\" is missing, download it from \"https://github.com/TrinityCore/TrinityCore/releases\"" \
    ...
    // Update database
    FMT_LOG_INFO("sql.updates", ">> Applying \'{}\'...", base.generic_string());
    try
    {
        ApplyFile(pool, base);
    }
    ...
    Path const& path)
{
    ...
    if (ret != EXIT_SUCCESS)
    {
        FMT_LOG_FATAL("sql.updates", "Applying of file \'{}\' to database \'{}\' failed!" \
