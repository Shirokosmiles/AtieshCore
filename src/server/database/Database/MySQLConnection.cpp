...
uint32 MySQLConnection::Open()
{
    ...
    if (!mysqlInit)
    {
        FMT_LOG_ERROR("sql.sql", "Could not initialize Mysql connection to database `{}`", m_connectionInfo.database);
    ...
    if (m_Mysql)
    {
        if (!m_reconnecting)
        {
            FMT_LOG_INFO("sql.sql", "MySQL client library: {}", mysql_get_client_info());
            FMT_LOG_INFO("sql.sql", "MySQL server ver: {} ", mysql_get_server_info(m_Mysql));
            // MySQL version above 5.1 IS required in both client and server and there is no known issue with different versions above 5.1
            // if (mysql_get_server_version(m_Mysql) != mysql_get_client_version())
            //     FMT_LOG_INFO("sql.sql", "[WARNING] MySQL client/server version mismatch; may conflict with behaviour of prepared statements.");
        }

        FMT_LOG_INFO("sql.sql", "Connected to MySQL database at {}", m_connectionInfo.host);
    ...
    {
        FMT_LOG_ERROR("sql.sql", "Could not connect to MySQL database at {}: {}", m_connectionInfo.host, mysql_error(mysqlInit));
...
bool MySQLConnection::Execute(char const* sql)
{
...

    {
        ...
        if (mysql_query(m_Mysql, sql))
        {
            ...
            FMT_LOG_INFO("sql.sql", "SQL: {}", sql);
            FMT_LOG_ERROR("sql.sql", "[{}] {}", lErrno, mysql_error(m_Mysql));
            ...
            FMT_LOG_DEBUG("sql.sql", "[{} ms] SQL: {}", getMSTimeDiff(_s, getMSTime()), sql);
    }
...
bool MySQLConnection::Execute(PreparedStatementBase* stmt)
{
    ...
    FMT_LOG_DEBUG("sql.sql", "[{} ms] SQL(p): {}", getMSTimeDiff(_s, getMSTime()), m_mStmt->getQueryString());
    ...
    for (auto itr = queries.begin(); itr != queries.end(); ++itr)
    {
        ...
        switch (itr->type)
        {
            case SQL_ELEMENT_PREPARED:
            {
                ...
                if (!Execute(stmt))
                {
                    FMT_LOG_WARN("sql.sql", "Transaction aborted. {} queries not executed.", (uint32)queries.size());
...
MySQLPreparedStatement* MySQLConnection::GetPreparedStatement(uint32 index)
{
    ...
    if (!ret)
        FMT_LOG_ERROR("sql.sql", "Could not fetch prepared statement {} on database `{}`, connection type: {}.",
...
void MySQLConnection::PrepareStatement(uint32 index, std::string const& sql, ConnectionFlags flags)
{
    ...
    if (!stmt)
    {
        FMT_LOG_ERROR("sql.sql", "In mysql_stmt_init() id: {}, sql: \"{}\"", index, sql);
        FMT_LOG_ERROR("sql.sql", "{}", mysql_error(m_Mysql));
        m_prepareError = true;
    }
    else
    {
        ...
        {
            FMT_LOG_ERROR("sql.sql", "In mysql_stmt_prepare() id: {}, sql: \"{}\"", index, sql);
            FMT_LOG_ERROR("sql.sql", "{}", mysql_stmt_error(stmt));
...
bool MySQLConnection::_HandleMySQLErrno(uint32 errNo, uint8 attempts /*= 5*/)
{
    switch (errNo)
    {
        ...
        case CR_SERVER_LOST_EXTENDED:
        {
            if (m_Mysql)
            {
                FMT_LOG_ERROR("sql.sql", "Lost the connection to the MySQL server!");
        ...
        {
            FMT_LOG_INFO("sql.sql", "Attempting to reconnect to the MySQL server...");
            ...
            if (!lErrno)
            {
                ...
                {
                    FMT_LOG_FATAL("sql.sql", "Could not re-prepare statements!");
                    std::this_thread::sleep_for(std::chrono::seconds(10));
                    std::abort();
                }

                FMT_LOG_INFO("sql.sql", "Successfully reconnected to {} @{}:{} ({}).",
            ...
            if ((--attempts) == 0)
            {
                ...
                FMT_LOG_FATAL("sql.sql", "Failed to reconnect to the MySQL server, "
        ...
        case ER_NO_SUCH_TABLE:
            FMT_LOG_ERROR("sql.sql", "Your database structure is not up to date. Please make sure you've executed all queries in the sql/updates folders.");
            std::this_thread::sleep_for(std::chrono::seconds(10));
            std::abort();
            return false;
        case ER_PARSE_ERROR:
            FMT_LOG_ERROR("sql.sql", "Error while parsing SQL. Core fix required.");
            std::this_thread::sleep_for(std::chrono::seconds(10));
            std::abort();
            return false;
        default:
            FMT_LOG_ERROR("sql.sql", "Unhandled MySQL errno {}. Unexpected behaviour possible.", errNo);
            return false;
    }
}
