...
uint32 DatabaseWorkerPool<T>::Open()
{
    ...
    FMT_LOG_INFO("sql.driver", "Opening DatabasePool '{}'. "
...
{
    FMT_LOG_INFO("sql.driver", "Closing down DatabasePool '{}'.", GetDatabaseName());

    //! Closes the actualy MySQL connection.
    _connections[IDX_ASYNC].clear();

    FMT_LOG_INFO("sql.driver", "Asynchronous connections on DatabasePool '{}' terminated. "
    ...
    FMT_LOG_INFO("sql.driver", "All connections on DatabasePool '{}' closed.", GetDatabaseName());
}
...
void DatabaseWorkerPool<T>::CommitTransaction(SQLTransaction<T> transaction)
{
    ...
    {
    case 0:
        FMT_LOG_DEBUG("sql.driver", "Transaction contains 0 queries. Not executing.");
        return;
    case 1:
        FMT_LOG_DEBUG("sql.driver", "Warning: Transaction only holds 1 query, consider removing Transaction context in code.");
        break;
    default:
        break;
    }
...
uint32 DatabaseWorkerPool<T>::OpenConnections(InternalIndex type, uint8 numConnections)
{
    for (uint8 i = 0; i < numConnections; ++i)
    {
        ...
        {
#ifndef LIBMARIADB
            FMT_LOG_INFO("sql.driver", "TrinityCore does not support MySQL versions below " MIN_MYSQL_SERVER_VERSION_STRING " (found id {}, need id >= {}), please update your MySQL server", connection->GetServerVersion(), MIN_MYSQL_SERVER_VERSION);
#else
            FMT_LOG_INFO("sql.driver", "TrinityCore does not support MariaDB versions below " MIN_MARIADB_SERVER_VERSION_STRING " (found id {}, need id >= {}), please update your MySQL server", connection->GetServerVersion(), MIN_MARIADB_SERVER_VERSION);
#endif
...
T* DatabaseWorkerPool<T>::GetFreeConnection()
{
    ...
    if (_warnSyncQueries)
    {
        ...
        FMT_LOG_WARN("sql.performances", "Sync query at:\n{}", ss.str());
    }
#endif
