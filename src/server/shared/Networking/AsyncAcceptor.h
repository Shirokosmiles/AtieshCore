...
public:
    ...
    void AsyncAcceptWithCallback()
    {
        ...
        _acceptor.async_accept(*socket, [this, socket, threadIndex](boost::system::error_code error)
        {
            if (!error)
            {
                ...
                {
                    FMT_LOG_INFO("network", "Failed to initialize client's socket {}", err.what());
                }
            }
    ...
    bool Bind()
    {
        ...
        if (errorCode)
        {
            FMT_LOG_INFO("network", "Failed to open acceptor {}", errorCode.message());
            return false;
        }
        ...
        if (errorCode)
        {
            FMT_LOG_INFO("network", "Failed to set reuse_address option on acceptor {}", errorCode.message());
            return false;
        }
#endif
        ...
        if (errorCode)
        {
            FMT_LOG_INFO("network", "Could not bind to {}:{} {}", _endpoint.address().to_string(), _endpoint.port(), errorCode.message());
            return false;
        }
        ...
        if (errorCode)
        {
            FMT_LOG_INFO("network", "Failed to start listening on {}:{} {}", _endpoint.address().to_string(), _endpoint.port(), errorCode.message());
            return false;
        }
...
void AsyncAcceptor::AsyncAccept()
{
    _acceptor.async_accept(_socket, [this](boost::system::error_code error)
    {
        if (!error)
        {
            ...
            {
                FMT_LOG_INFO("network", "Failed to retrieve client's remote address {}", err.what());
            }
        }
