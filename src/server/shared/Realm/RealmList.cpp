...
void RealmList::UpdateRealms(boost::system::error_code const& error)
{
    ...
    FMT_LOG_DEBUG("server.authserver", "Updating Realm List...");
    ...
    if (result)
    {
        do
        {
            try
            {
                ...
                if (!externalAddress)
                {
                    FMT_LOG_ERROR("server.authserver", "Could not resolve address {} for realm \"{}\" id {}", externalAddressString, name, realmId);
                    continue;
                }
                ...
                if (!localAddress)
                {
                    FMT_LOG_ERROR("server.authserver", "Could not resolve localAddress {} for realm \"{}\" id {}", localAddressString, name, realmId);
                    continue;
                }
                ...
                if (!localSubmask)
                {
                    FMT_LOG_ERROR("server.authserver", "Could not resolve localSubnetMask {} for realm \"{}\" id {}", localSubmaskString, name, realmId);
                    continue;
                }
                ...
                if (!existingRealms.count(id))
                    FMT_LOG_INFO("server.authserver", "Added realm \"{}\" at {}:{}.", name, externalAddressString, port);
                else
                    FMT_LOG_DEBUG("server.authserver", "Updating realm \"{}\" at {}:{}.", name, externalAddressString, port);

                existingRealms.erase(id);
            }
            catch (std::exception& ex)
            {
                FMT_LOG_ERROR("server.authserver", "Realmlist::UpdateRealms has thrown an exception: {}", ex.what());
                ABORT();
            }
        }
        while (result->NextRow());
    ...
    for (auto itr = existingRealms.begin(); itr != existingRealms.end(); ++itr)
        FMT_LOG_INFO("server.authserver", "Removed realm \"{}\".", itr->second);
