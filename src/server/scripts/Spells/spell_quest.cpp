    ...
    public:
        ...
        class spell_q11010_q11102_q11023_choose_loc_SpellScript : public SpellScript
        {
            ...
            void HandleDummy(SpellEffIndex /*effIndex*/)
            {
                ...
                Trinity::PlayerListSearcher<Trinity::AnyPlayerInObjectRangeCheck> searcher(caster, _players, checker);
                Cell::VisitWorldObjects(caster, searcher, 65.0f);
                for (auto const& pointer : _players)
                {
                    if (pointer->HasAuraType(SPELL_AURA_FLY) ||
                        pointer->HasAuraType(SPELL_AURA_MOD_INCREASE_MOUNTED_FLIGHT_SPEED))
                        // Summom Fel Cannon (bunny version) at found player
                        caster->SummonCreature(NPC_FEL_CANNON2, pointer->GetPositionX(), pointer->GetPositionY(), pointer->GetPositionZ());
                }
                _players.clear();
            }
