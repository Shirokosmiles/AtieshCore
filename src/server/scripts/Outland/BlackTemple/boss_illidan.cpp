		...
		struct boss_illidan_stormrageAI : public BossAI
		{
            ...
            void DamageTaken(Unit* /*who*/, uint32& damage) override
			{
				if (damage >= me->GetHealth())
				{
					...
					{
                        if (Creature* maiev = ObjectAccessor::GetCreature(*me, _maiev))
                        {
							maiev->StopMoving();
			...
			void UpdateAI(uint32 diff) override
			{
				...
				switch (events2.ExecuteEvent())
				{
					...
					case EVENT_PHASE_2_EYE_BEAM_START:
						...
						if (Creature* trigger = ObjectAccessor::GetCreature(*me, _eyetarget))
							trigger->GetMotionMaster()->MovePoint(0, eyeBeamPos[beamPosId+MAX_EYE_BEAM_POS]);
						break;
				...
				switch (uint32 eventId = events.ExecuteEvent())
				{
					...
					case EVENT_PHASE_2_SUMMON1:
						...
						me->GetMotionMaster()->MovePoint(POINT_ILLIDAN_HOVER, 727.875f, 305.365f, 356.0f);
						break;
					...
					case EVENT_PHASE_2_CHANGE_POS:
						...
						me->GetMotionMaster()->MovePoint(POINT_ILLIDAN_HOVER, airHoverPos[beamPosId]);
						break;
...
class npc_akama_illidan : public CreatureScript
{
	public:
		...
		{
			npc_akama_illidanAI(Creature* creature) : npc_escortAI(creature), _summons(me)
			{
                _instance = creature->GetInstanceScript();
				if (_instance->GetBossState(DATA_AKAMA) == DONE)
				{
            ...
            void DoAction(int32 param) override
			{
				...
				{
					me->CombatStop(true);
                    _events.ScheduleEvent(EVENT_AKAMA_SUMMON_ILLIDARI, 8000);
                    _events.ScheduleEvent(EVENT_AKAMA_SPELL_CHAIN, 7000);
                    _events.ScheduleEvent(EVENT_AKAMA_HEALTH, 1000);
            ...
            void Reset() override
			{
				...
				me->setActive(false);
				me->SetVisible(_instance->GetBossState(DATA_ILLIDARI_COUNCIL) == DONE && _instance->GetBossState(DATA_ILLIDAN_STORMRAGE) != DONE);
                _events.Reset();
                _summons.DespawnAll();
			}
			...
			void sGossipSelect(Player* player, uint32 /*sender*/, uint32 action)
			{
				...
				else
				{
                    ...
                    _events.Reset();
                    _events.ScheduleEvent(EVENT_AKAMA_SCENE_20, 5000);
                    _events.ScheduleEvent(EVENT_AKAMA_SCENE_21, 8000);
                    _events.ScheduleEvent(EVENT_AKAMA_SCENE_22, 10000);
                    _events.ScheduleEvent(EVENT_AKAMA_SCENE_23, 23000);
                    _events.ScheduleEvent(EVENT_AKAMA_SCENE_24, 34000);
                    _events.ScheduleEvent(EVENT_AKAMA_SCENE_25, 41000);
                    _events.ScheduleEvent(EVENT_AKAMA_SCENE_26, 46000);
                    _events.ScheduleEvent(EVENT_AKAMA_SCENE_27, 49000);
                    _events.ScheduleEvent(EVENT_AKAMA_SCENE_28, 49200);
                    _events.ScheduleEvent(EVENT_AKAMA_SCENE_29, 52000);
				}
			}
			...
			void WaypointReached(uint32 pointId)
			{
				if (pointId == POINT_DOORS)
				{
                    ...
                    _events.ScheduleEvent(EVENT_AKAMA_SCENE_1, 0);
                    _events.ScheduleEvent(EVENT_AKAMA_SCENE_2, 4000);
                    _events.ScheduleEvent(EVENT_AKAMA_SCENE_3, 7000);
                    _events.ScheduleEvent(EVENT_AKAMA_SCENE_4, 17000);
                    _events.ScheduleEvent(EVENT_AKAMA_SCENE_5, 23000);
                    _events.ScheduleEvent(EVENT_AKAMA_SCENE_6, 25000);
                    _events.ScheduleEvent(EVENT_AKAMA_SCENE_7, 31000);
                    _events.ScheduleEvent(EVENT_AKAMA_SCENE_8, 40000);
                    _events.ScheduleEvent(EVENT_AKAMA_SCENE_9, 54000);
                    _events.ScheduleEvent(EVENT_AKAMA_SCENE_10, 57000);
                    _events.ScheduleEvent(EVENT_AKAMA_SCENE_11, 62000);
				}
			...
			void UpdateEscortAI(uint32 diff) override
			{
				...
				switch (_events.ExecuteEvent())
				{
					...
					case EVENT_AKAMA_SCENE_9:
                        _instance->SetBossState(DATA_AKAMA, NOT_STARTED);
                        _instance->SetBossState(DATA_AKAMA, DONE);
						break;
					...
					case EVENT_AKAMA_SCENE_20:
						if (Creature* illidan = ObjectAccessor::GetCreature(*me, _instance->GetGuidData(NPC_ILLIDAN_STORMRAGE)))
					...
					case EVENT_AKAMA_SUMMON_ILLIDARI:
                        ...
                        _events.ScheduleEvent(EVENT_AKAMA_SUMMON_ILLIDARI, urand(2000, 6000));
						break;
					case EVENT_AKAMA_SPELL_CHAIN:
						if (me->GetVictim())
							me->CastSpell(me->GetVictim(), SPELL_CHAIN_LIGHTNING, false);
                        _events.ScheduleEvent(EVENT_AKAMA_SPELL_CHAIN, 20000);
						break;
