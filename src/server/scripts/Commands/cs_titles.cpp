...
public:
    ...
    static bool HandleTitlesCurrentCommand(ChatHandler* handler, Variant<Hyperlink<title>, uint16> titleId)
    {
        ...
        std::string titleNameStr = Trinity::StringFormat(target->GetNativeGender() == GENDER_MALE ? titleInfo->Name[handler->GetSessionDbcLocale()].c_str() : titleInfo->Name1[handler->GetSessionDbcLocale()].c_str(), target->GetName());
