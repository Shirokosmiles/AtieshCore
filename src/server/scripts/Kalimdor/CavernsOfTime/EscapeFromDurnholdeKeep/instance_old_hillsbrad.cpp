...
public:
    ...
    struct instance_old_hillsbrad_InstanceMapScript : public InstanceScript
    {
        ...
        Player* GetPlayerInMap()
        {
            ...
            FMT_LOG_DEBUG("scripts", "Instance Old Hillsbrad: GetPlayerInMap, but PlayerList is empty!");
            return nullptr;
        }
        ...
        void SetData(uint32 type, uint32 data) override
        {
            ...
            if (!player)
            {
                FMT_LOG_DEBUG("scripts", "Instance Old Hillsbrad: SetData (Type: {} Data {}) cannot find any player.", type, data);
                return;
            }
            ...
            switch (type)
            {
                case TYPE_BARREL_DIVERSION:
                {
                    if (data == IN_PROGRESS)
                    {
                        ...
                        FMT_LOG_DEBUG("scripts", "Instance Old Hillsbrad: go_barrel_old_hillsbrad count {}", mBarrelCount);
                ...
                case TYPE_THRALL_EVENT:
                {
                    if (data == FAIL)
                    {
                        if (mThrallEventCount <= 20)
                        {
                            ...
                            FMT_LOG_DEBUG("scripts", "Instance Old Hillsbrad: Thrall event failed {} times. Resetting all sub-events.", mThrallEventCount);
                        ...
                        m_auiEncounter[1] = data;
                    FMT_LOG_DEBUG("scripts", "Instance Old Hillsbrad: Thrall escort event adjusted to data {}.", data);
                    break;
                }
                case TYPE_THRALL_PART1:
                    m_auiEncounter[2] = data;
                    FMT_LOG_DEBUG("scripts", "Instance Old Hillsbrad: Thrall event part I adjusted to data {}.", data);
                    break;
                case TYPE_THRALL_PART2:
                    m_auiEncounter[3] = data;
                    FMT_LOG_DEBUG("scripts", "Instance Old Hillsbrad: Thrall event part II adjusted to data {}.", data);
                    break;
                case TYPE_THRALL_PART3:
                    m_auiEncounter[4] = data;
                    FMT_LOG_DEBUG("scripts", "Instance Old Hillsbrad: Thrall event part III adjusted to data {}.", data);
                    break;
                case TYPE_THRALL_PART4:
                    m_auiEncounter[5] = data;
                     FMT_LOG_DEBUG("scripts", "Instance Old Hillsbrad: Thrall event part IV adjusted to data {}.", data);
                    break;
            }
        }
