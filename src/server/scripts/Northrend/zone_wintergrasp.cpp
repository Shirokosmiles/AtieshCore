...
private:
    bool CanBuild() const
    {
        SpecialEvent* wintergrasp = sSpecialEventMgr->GetEnabledSpecialEventByZoneId(me->GetZoneId());
        if (!wintergrasp || !wintergrasp->IsActiveTime())
            return false;
...
private:
    void HandleTeleport(Creature* vehicle, Creature* teleportTrigger, SpecialEvent* wintergrasp)
    {
        ...
        if (wintergrasp && wintergrasp->GetAttackingTeamId() != owner->GetTeamId())
            return;
...
class spell_wintergrasp_defender_teleport : public SpellScript
{
    ...
    SpellCastResult CheckCast()
    {
        if (Player * target = GetExplTargetUnit()->ToPlayer())
        {
            ...
            {
                if (SpecialEvent* wintergrasp = sSpecialEventMgr->GetEnabledSpecialEventByZoneId(target->GetZoneId()))
                {
...
class spell_wintergrasp_waiting_to_resurrect : public AuraScript
{
    ...
    void HandleEffectRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
    {
        ...
        if (SpecialEvent * wintergrasp = sSpecialEventMgr->GetEnabledSpecialEventByZoneId(owner->GetZoneId()))
...
public:
    ...
    {
        SpecialEvent* wintergrasp = sSpecialEventMgr->GetEnabledSpecialEventByEventId(SPECIALEVENT_EVENTID_WINTERGRASP);
