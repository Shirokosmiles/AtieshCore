...
void RASession::Start()
{
    ...
    FMT_LOG_INFO("commands.ra", "Accepting RA connection from user {} (IP: {})", username, GetRemoteIpAddress());
    ...
    FMT_LOG_INFO("commands.ra", "User {} (IP: {}) authenticated correctly to RA", username, GetRemoteIpAddress());
...
bool RASession::CheckAccessLevel(const std::string& user)
{
    ...
    if (!result)
    {
        FMT_LOG_INFO("commands.ra", "User {} does not exist in database", user);
        return false;
    }
    ...
    {
        FMT_LOG_INFO("commands.ra", "User {} has no privilege to login", user);
        return false;
    }
    else if (fields[2].GetInt32() != -1)
    {
        FMT_LOG_INFO("commands.ra", "User {} has to be assigned on all realms (with RealmID = '-1')", user);
        return false;
    }
...
bool RASession::CheckPassword(const std::string& user, const std::string& pass)
{
    ...
    FMT_LOG_INFO("commands.ra", "Wrong password for user: {}", user);
    return false;
}
...
bool RASession::ProcessCommand(std::string& command)
{
    ...
    FMT_LOG_INFO("commands.ra", "Received command: {}", command);
