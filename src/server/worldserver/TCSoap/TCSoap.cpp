...
void TCSoapThread(const std::string& host, uint16 port)
{
    ...
    {
        FMT_LOG_ERROR("network.soap", "Couldn't bind to {}:{}", host, port);
        exit(-1);
    }

    FMT_LOG_INFO("network.soap", "Bound to http://{}:{}", host, port);
    ...
    while (!World::IsStopped())
    {
        ...
        FMT_LOG_DEBUG("network.soap", "Accepted connection from IP={}.{}.{}.{}", (int)(soap.ip>>24)&0xFF, (int)(soap.ip>>16)&0xFF, (int)(soap.ip>>8)&0xFF, (int)soap.ip&0xFF);
...
{
    FMT_LOG_TRACE("network.soap", "SOAPWorkingThread::process_message");
...
int ns1__executeCommand(soap* soap, char* command, char** result)
{
    ...
    {
        FMT_LOG_INFO("network.soap", "Client didn't provide login information");
        return 401;
    }
    ...
    if (!accountId)
    {
        FMT_LOG_INFO("network.soap", "Client used invalid username '{}'", soap->userid);
        return 401;
    }

    if (!AccountMgr::CheckPassword(accountId, soap->passwd))
    {
        FMT_LOG_INFO("network.soap", "Invalid password for account '{}'", soap->userid);
        return 401;
    }

    if (AccountMgr::GetSecurity(accountId, realm.Id.Realm) < SEC_ADMINISTRATOR)
    {
        FMT_LOG_INFO("network.soap", "{}'s gmlevel is too low", soap->userid);
        return 403;
    }
    ...
    FMT_LOG_INFO("network.soap", "Received command '{}'", command);
