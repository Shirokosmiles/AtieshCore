...
_flags(AFLAG_NONE), _effectsToApply(effMask), _needClientUpdate(false)
{
    ...
    if (GetBase()->CanBeSentToClient())
    {
        ...
        if (slot < MAX_AURAS)
        {
            ...
            SetNeedClientUpdate();
            FMT_LOG_DEBUG("spells", "Aura: {} Effect: {} put to unit visible auras slot: {}", GetBase()->GetId(), GetEffectMask(), slot);
        }
        else
            FMT_LOG_ERROR("spells", "Aura: {} Effect: {} could not find empty unit visible slot", GetBase()->GetId(), GetEffectMask());
    }
...
void AuraApplication::_HandleEffect(uint8 effIndex, bool apply)
{
    ...
    FMT_LOG_DEBUG("spells", "AuraApplication::_HandleEffect: {}, apply: {}: amount: {}", aurEff->GetAuraType(), apply, aurEff->GetAmount());
...
void Aura::_UnapplyForTarget(Unit* target, Unit* caster, AuraApplication* auraApp)
{
    ...
    {
        FMT_LOG_ERROR("spells", "Aura::_UnapplyForTarget, target: {}, caster: {}, spell:{} was not found in owners application map!",
    ...
    for (auto itr = targets.begin(); itr != targets.end();)
    {
        ...
        else
        {
            ...
            if (!GetOwner()->IsSelfOrInSameMap(itr->first))
            {
                ...
                FMT_LOG_FATAL("spells", "Aura {}: Owner {} (map {}) is not in the same map as target {} (map {}).", GetSpellInfo()->Id,
                ...
                if (GetSpellInfo()->SpellFamilyFlags[2] & 0x2)
                {
                    ...
                    if (aurEff)
                    {
                        ...
                        switch (aurEff->GetId())
                        {
                            ...
                            default:
                                FMT_LOG_ERROR("spells", "Aura::HandleAuraSpecificMods: Unknown rank of Crypt Fever/Ebon Plague ({}) found", aurEff->GetId());
                        }
                ...
                if (GetSpellInfo()->SpellFamilyFlags[1] & 0x00000400)
                {
                    if (AuraEffect* aurEff = caster->GetAuraEffect(SPELL_AURA_DUMMY, SPELLFAMILY_WARLOCK, 98, 0))
                    {
                        ...
                        switch (aurEff->GetId())
                        {
                            ...
                            default:
                                FMT_LOG_ERROR("spells", "Aura::HandleAuraSpecificMods: Unknown rank of Improved Fear ({}) found", aurEff->GetId());
                        }
                        if (spellId)
...
void Aura::LoadScripts()
{
    ...
    for (auto itr = m_loadedScripts.begin(); itr != m_loadedScripts.end(); ++itr)
    {
        FMT_LOG_DEBUG("spells", "Aura::LoadScripts: Script `{}` for aura `{}` is loaded now", (*itr)->_GetScriptName()->c_str(), m_spellInfo->Id);
        (*itr)->Register();
    }
}
