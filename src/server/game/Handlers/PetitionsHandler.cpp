...
{
    FMT_LOG_DEBUG("network", "Received opcode CMSG_PETITION_BUY");
    ...
    FMT_LOG_DEBUG("network", "Petitioner {} tried sell petition: name {}", guidNPC.ToString(), name);

    // prevent cheating
    Creature* creature = GetPlayer()->GetNPCIfCanInteractWith(guidNPC, UNIT_NPC_FLAG_PETITIONER);
    if (!creature)
    {
        FMT_LOG_DEBUG("network", "WORLD: HandlePetitionBuyOpcode - {} not found or you can't interact with him.", guidNPC.ToString());
        return;
    }
    ...
    else
    {
        ...
        switch (clientIndex)                                 // arenaSlot+1 as received from client (1 from 3 case)
        {
            ...
            case 3:
                ...
                break;
            default:
                FMT_LOG_DEBUG("network", "unknown selection at buy arena petition: {}", clientIndex);
                return;
        }
    ...
    if (Petition const* petition = sPetitionMgr->GetPetitionByOwnerWithType(_player->GetGUID(), type))
    {
        ...
        FMT_LOG_DEBUG("network", "Invalid petition {}", petition->PetitionGuid.ToString());
    }
...
{
    FMT_LOG_DEBUG("network", "Received opcode CMSG_PETITION_SHOW_SIGNATURES");
    ...
    if (!petition)
    {
        FMT_LOG_DEBUG("entities.player.items", "Petition {} is not found for player {} {}", petitionGuid.ToString(), GetPlayer()->GetGUID().ToString(), GetPlayer()->GetName());
        return;
    }
    ...
    FMT_LOG_DEBUG("network", "CMSG_PETITION_SHOW_SIGNATURES petition {}", petitionGuid.ToString());
...
{
    FMT_LOG_DEBUG("network", "Received opcode CMSG_PETITION_QUERY");   // ok

    ObjectGuid::LowType guildguid;
    ObjectGuid petitionguid;
    recvData >> guildguid;                                 // in Trinity always same as GUID_LOPART(petitionguid)
    recvData >> petitionguid;                              // petition guid
    FMT_LOG_DEBUG("network", "CMSG_PETITION_QUERY Petition {} Guild GUID {}", petitionguid.ToString(), guildguid);
...
void WorldSession::SendPetitionQueryOpcode(ObjectGuid petitionguid)
{
    ...
    if (!petition)
    {
        FMT_LOG_DEBUG("network", "CMSG_PETITION_QUERY failed for petition ({})", petitionguid.ToString());
        return;
    }
...
{
    FMT_LOG_DEBUG("network", "Received opcode MSG_PETITION_RENAME");   // ok
    ...
    FMT_LOG_DEBUG("network", "Petition {} renamed to '{}'", petitionGuid.ToString(), newName);
...
{
    FMT_LOG_DEBUG("network", "Received opcode CMSG_PETITION_SIGN");    // ok
    ...
    if (!petition)
    {
        FMT_LOG_ERROR("network", "Petition {} is not found for player {} {}", petitionGuid.ToString(), GetPlayer()->GetGUID().ToString(), GetPlayer()->GetName());
        return;
    }
    ...
    FMT_LOG_DEBUG("network", "PETITION SIGN: {} by player: {} ({} Account: {})", petitionGuid.ToString(), _player->GetName(), playerGuid.ToString(), GetAccountId());
...
{
    FMT_LOG_DEBUG("network", "Received opcode MSG_PETITION_DECLINE");  // ok

    ObjectGuid petitionguid;
    recvData >> petitionguid;                              // petition guid
    FMT_LOG_DEBUG("network", "Petition {} declined by {}", petitionguid.ToString(), _player->GetGUID().ToString());
...
{
    FMT_LOG_DEBUG("network", "Received opcode CMSG_OFFER_PETITION");   // ok
    ...
    FMT_LOG_DEBUG("network", "OFFER PETITION: type {}, {}, to {}", static_cast<uint32>(type), petitionGuid.ToString(), offererGuid.ToString());
...
{
    FMT_LOG_DEBUG("network", "Received opcode CMSG_TURN_IN_PETITION");
    ...
    FMT_LOG_DEBUG("network", "Petition {} turned in by {}", petitionGuid.ToString(), _player->GetGUID().ToString());

    Petition const* petition = sPetitionMgr->GetPetition(petitionGuid);
    if (!petition)
    {
        FMT_LOG_ERROR("entities.player.cheat", "Player {} {} tried to turn in petition ({}) that is not present in the database", _player->GetName(), _player->GetGUID().ToString(), petitionGuid.ToString());
        return;
    }
    ...
    else
    {
        ...
        FMT_LOG_DEBUG("network", "PetitonsHandler: Arena team (guid: {}) added to ObjectMgr", arenaTeam->GetId());

        // Add members
        for (Signature const& signature : signatures)
        {
            FMT_LOG_DEBUG("network", "PetitionsHandler: Adding arena team (guid: {}) member {}", arenaTeam->GetId(), signature.second.ToString());
    ...
    // created
    FMT_LOG_DEBUG("network", "Player {} ({}) turning in petition {}", _player->GetName(), _player->GetGUID().ToString(), petitionGuid.ToString());
...
{
    FMT_LOG_DEBUG("network", "Received CMSG_PETITION_SHOWLIST");
...
void WorldSession::SendPetitionShowList(ObjectGuid guid)
{
    ...
    if (!creature)
    {
        FMT_LOG_DEBUG("network", "WORLD: HandlePetitionShowListOpcode - {} not found or you can't interact with him.", guid.ToString());
        return;
    }
    ...
    SendPacket(&data);
    FMT_LOG_DEBUG("network", "Sent SMSG_PETITION_SHOWLIST");
}
