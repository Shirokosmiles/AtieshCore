...
void CharacterCache::LoadCharacterCacheStorage()
{
    ...
    if (!result)
    {
        FMT_LOG_INFO("server.loading", "No character name data loaded, empty query");
        return;
    }
    ...
    FMT_LOG_INFO("server.loading", "Loaded character infos for {} characters in {} ms", _characterCacheStore.size(), GetMSTimeDiffToNow(oldMSTime));
}
