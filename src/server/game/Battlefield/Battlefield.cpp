...
void Battlefield::HandlePlayerLeaveZone(Player* player, uint32 /*zone*/)
{
    if (PlayerInBFPlayerMap(player))
    {
        ...
        else
        {
            ...
            PlayerHolderContainer::const_iterator itr = m_PlayerMap.find(playerID);
...
bool Battlefield::Update(uint32 diff)
{
    ...
    if (m_uiAcceptTimer <= diff)
    {
        ...
        std::set<ObjectGuid::LowType>_playerkick;
...
{
    uint32 playerID = plr->GetGUID().GetCounter();
    PlayerHolderContainer::const_iterator itr = m_PlayerMap.find(playerID);
    if (itr != m_PlayerMap.end())
        return true;
...
{
    uint32 playerID = plr->GetGUID().GetCounter();
    PlayerHolderContainer::iterator itr = m_PlayerMap.find(playerID);
    if (itr != m_PlayerMap.end())
...
void Battlefield::AddPlayer(Player* plr, bool InZone, bool IsWaitingQueue, bool IsWaitingWar, bool IsWaitingKick, time_t time)
{
    ...
    pl.time = time;
    m_PlayerMap[plr->GetGUID().GetCounter()] = pl;
}
...
void Battlefield::KickPlayerFromBattlefield(ObjectGuid::LowType guid)
{
...
void Battlefield::HandleResurrectedPlayer(ObjectGuid guid)
{
    for (auto& graveID : m_graveyardMap)
    {
        if (BfGraveyard* grave = graveID.second)
...
void BfGraveyard::Resurrect()
{
    ...
    for (auto itr = _resurrectQueue.begin(); itr != _resurrectQueue.end(); ++itr)
    {
        ...
        if (player->IsInWorld())
            if (Creature* spirit = _Bf->GetCreature(_SpiritGuide[_ControlTeam]))
...
void BfGraveyard::RelocateDeadPlayers()
{
    ...
    for (auto const& Tguid : teleportSet)
    {
        if (Player* player = ObjectAccessor::FindConnectedPlayer(Tguid))
        {
            if (!closestGrave)
            {
                closestGrave = ASSERT_NOTNULL(_Bf->GetClosestGraveyard(player));
...
bool BfGraveyard::HasNpc(ObjectGuid guid)
{
    ...
    if (!_Bf->GetCreature(_SpiritGuide[TEAM_ALLIANCE]) ||
        !_Bf->GetCreature(_SpiritGuide[TEAM_HORDE]))
        return false;
