...
void PetitionMgr::LoadPetitions()
{
    ...
    if (!result)
    {
        FMT_LOG_INFO("server.loading", ">> Loaded 0 petitions.");
        return;
    }
    ...
    FMT_LOG_INFO("server.loading", ">> Loaded {} petitions in: {} ms.", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void PetitionMgr::LoadSignatures()
{
    ...
    if (!result)
    {
        FMT_LOG_INFO("server.loading", ">> Loaded 0 Petition signs!");
        return;
    }
    ...
    FMT_LOG_INFO("server.loading", ">> Loaded {} Petition signs in {} ms.", count, GetMSTimeDiffToNow(oldMSTime));
}
