...
Position const BG_IC_HangarTrigger[MAX_TRIGGER_SPAWNS_PER_FACTION * PVP_TEAMS_COUNT] =
{
    { 11.69965f,  0.034146f, 20.62076f, 3.211406f },
    { 7.305609f, -0.095246f, 34.51022f, 3.159046f }
};

Position const BG_IC_HangarCaptains[MAX_CAPTAIN_SPAWNS_PER_FACTION * PVP_TEAMS_COUNT] =
{
...
uint32 const BG_IC_GraveyardIds[MAX_NODE_TYPES + PVP_TEAMS_COUNT] = { 0, 0, 1480, 1481, 1482, 1485, 1486, 1483, 1484 };

Position const BG_IC_SpiritGuidePos[MAX_NODE_TYPES + PVP_TEAMS_COUNT] =
{
    ...
    for (uint8 i = NODE_TYPE_REFINERY; i < MAX_NODE_TYPES; ++i)
    {
        ...
        if (currentNode->needChange)
        {
            if (currentNode->timer <= diff)
            {
                ...
                if (currentNode->faction == TEAM_ALLIANCE)
                    SendBroadcastText(currentNode->string, CHAT_MSG_BG_SYSTEM_ALLIANCE);
                else
                    SendBroadcastText(currentNode->string, CHAT_MSG_BG_SYSTEM_HORDE);
...
{
    packet.Worldstates.emplace_back(BG_IC_WS_REINFORCEMENTS_SHOW_A, 1);
    packet.Worldstates.emplace_back(BG_IC_WS_REINFORCEMENTS_SHOW_H, 1);
    packet.Worldstates.emplace_back(BG_IC_WS_ALLIANCE_REINFORCEMENTS, factionReinforcements[TEAM_ALLIANCE]);
    packet.Worldstates.emplace_back(BG_IC_WS_HORDE_REINFORCEMENTS, factionReinforcements[TEAM_HORDE]);
...
bool BattlegroundIC::SetupBattleground()
{
    for (uint8 i = 0; i <= BG_IC_GO_DOODAD_ND_WINTERORC_WALL_GATEFX_DOOR03; ++i)
    {
        ...
        {
            FMT_LOG_ERROR("bg.battleground", "Isle of Conquest: There was an error spawning gameobject {}. Battleground not Created!", object.Entry);
            return false;
        }
    }
    ...
    for (uint8 i = BG_IC_GO_TELEPORTER_1_1; i < BG_IC_GO_TELEPORTER_1_1 + MAX_FORTRESS_TELEPORTERS_SPAWNS; ++i)
    {
        ...
        if (!teleporter)
        {
            FMT_LOG_ERROR("bg.battleground", "Isle of Conquest: There was an error spawning gameobject {}. Battleground not Created!", teleportSpawn.Entry);
            return false;
        }
    ...
    for (uint8 i = BG_IC_GO_TELEPORTER_EFFECTS_A_1; i < BG_IC_GO_TELEPORTER_EFFECTS_A_1 + MAX_FORTRESS_TELEPORTER_EFFECTS_SPAWNS; ++i)
    {
        ...
        {
            FMT_LOG_ERROR("bg.battleground", "Isle of Conquest: There was an error spawning gameobject {}. Battleground not Created!", teleporterEffect.Entry);
            return false;
        }
    }
    ...
    for (uint8 i = BG_IC_GO_HANGAR_TELEPORTER_1_A; i < BG_IC_GO_HANGAR_TELEPORTER_1_A + MAX_HANGAR_TELEPORTERS_SPAWNS; ++i)
    {
        ...
        {
            FMT_LOG_ERROR("bg.battleground", "Isle of Conquest: There was an error spawning gameobject {}. Battleground not Created!", teleporter.Entry);
            return false;
        }
    }
    ...
    for (uint8 i = BG_IC_NPC_KOR_KRON_GUARD_1; i <= BG_IC_NPC_KEEP_CANNON_24; ++i)
    {
        ...
        {
            FMT_LOG_ERROR("bg.battleground", "Isle of Conquest: There was an error spawning creature {}", creature.Entry);
            return false;
        }
    }
    ...
    {
        FMT_LOG_ERROR("bg.battleground", "Isle of Conquest: Failed to spawn initial spirit guide!");
        return false;
    }
    ...
    {
        FMT_LOG_ERROR("bg.battleground", "Isle of Conquest: There was an error creating gunships!");
        return false;
    }
...
{
    //TODO SendMessage2ToAll(LANG_BG_IC_TEAM_WINS, CHAT_MSG_BG_SYSTEM_NEUTRAL, nullptr, (winner == ALLIANCE ? LANG_BG_IC_ALLIANCE : LANG_BG_IC_HORDE));
...
void BattlegroundIC::EventPlayerClickedOnFlag(Player* player, GameObject* target_obj)
{
    ...
    if (nextBannerId == currentNode->bannerIds[NODE_STATE_CONFLICT_A] || nextBannerId == currentNode->bannerIds[NODE_STATE_CONFLICT_H])
    {
        ...
        if (currentNode->faction == TEAM_ALLIANCE)
            SendBroadcastText(currentNode->string, CHAT_MSG_BG_SYSTEM_ALLIANCE, player);
        else
            SendBroadcastText(currentNode->string, CHAT_MSG_BG_SYSTEM_HORDE, player);
    ...
    else if (nextBannerId == currentNode->bannerIds[NODE_STATE_CONTROLLED_A] || nextBannerId == currentNode->bannerIds[NODE_STATE_CONTROLLED_H]) // if we are going to spawn the definitve faction banner, we dont need the timer anymore
    {
        currentNode->timer = 0;
        currentNode->needChange = false;
        SendBroadcastText(currentNode->string, CHAT_MSG_BG_SYSTEM_NEUTRAL, player);
...
uint32 BattlegroundIC::GetNextBanner(ICNodePoint const* node, TeamId team, bool returnDefinitve)
{
    ...
    FMT_LOG_ERROR("bg.battleground", "Isle Of Conquest: Unexpected return in GetNextBanner function (node {})", node->gameobjectId);
    return 0;
}
...
void BattlegroundIC::HandleCapturedNodes(ICNodePoint const* node, bool recapture)
{
    if (node->nodeType > NODE_TYPE_QUARRY)
    {
        if (!AddSpiritGuide(BG_IC_NPC_SPIRIT_GUIDE_1 + node->nodeType - (NODE_TYPE_QUARRY + 1), BG_IC_SpiritGuidePos[node->nodeType], node->faction))
            FMT_LOG_ERROR("bg.battleground", "Isle of Conquest: Failed to spawn spirit guide! point: {}, team: {}, ", node->nodeType, node->faction);
    }
    ...
    switch (node->gameobjectId)
    {
        ...
        case BG_IC_GO_HANGAR_BANNER_CONT_H:
        {
            ...
            for (uint8 u = BG_IC_GO_HANGAR_TELEPORTER_1_A; u < BG_IC_GO_HANGAR_TELEPORTER_1_A + MAX_HANGAR_TELEPORTERS_SPAWNS; u += PVP_TEAMS_COUNT)
            {
            ...
            for (uint8 u = BG_IC_GO_HANGAR_TELEPORTER_EFFECT_1_A; u < BG_IC_GO_HANGAR_TELEPORTER_EFFECT_1_A + MAX_HANGAR_TELEPORTER_EFFECTS_SPAWNS; u += PVP_TEAMS_COUNT)
            {
                SpawnBGObject(u + TEAM_ALLIANCE, node->faction == TEAM_ALLIANCE ? RESPAWN_IMMEDIATELY : RESPAWN_ONE_DAY);
                SpawnBGObject(u + TEAM_HORDE,    node->faction == TEAM_ALLIANCE ? RESPAWN_ONE_DAY : RESPAWN_IMMEDIATELY);
            }
            ...
            for (uint8 u = 0; u < MAX_TRIGGER_SPAWNS_PER_FACTION; ++u)
            {
                ...
                if (!AddCreature(NPC_WORLD_TRIGGER_NOT_FLOATING, BG_IC_NPC_WORLD_TRIGGER_NOT_FLOATING, BG_IC_HangarTrigger[node->faction], RESPAWN_ONE_DAY, node->faction == TEAM_ALLIANCE ? gunshipAlliance : gunshipHorde))
                    FMT_LOG_ERROR("bg.battleground", "Isle of Conquest: There was an error spawning a world trigger. Type: {}", BG_IC_NPC_WORLD_TRIGGER_NOT_FLOATING);
            }
            ...
            for (uint8 u = 0; u < MAX_CAPTAIN_SPAWNS_PER_FACTION; ++u)
            {
                ...
                if (type == BG_IC_NPC_GUNSHIP_CAPTAIN_2)
                    if (!AddCreature(node->faction == TEAM_ALLIANCE ? NPC_ALLIANCE_GUNSHIP_CAPTAIN : NPC_HORDE_GUNSHIP_CAPTAIN, type, BG_IC_HangarCaptains[node->faction == TEAM_ALLIANCE ? 3 : 1], RESPAWN_ONE_DAY, node->faction == TEAM_ALLIANCE ? gunshipAlliance : gunshipHorde))
                        FMT_LOG_ERROR("bg.battleground", "Isle of Conquest: There was an error spawning a world trigger. Type: {}", BG_IC_NPC_GUNSHIP_CAPTAIN_2);
            }
...
Creature* BattlegroundIC::SpawnSiegeEngine(TeamId team)
{
    ...
    if (!creature->Create(map->GenerateLowGuid<HighGuid::Vehicle>(), map, PHASEMASK_NORMAL, entry, pos))
    {
        delete creature;
        return nullptr;
    }
        ...
        case GO_HORDE_GATE_3:
            if (!GetBGCreature(BG_IC_NPC_OVERLORD_AGMAR, false))
            {
                ...
                if (!AddCreature(agmar.Entry, BG_IC_NPC_OVERLORD_AGMAR, agmar.Pos, agmar.SpawnTime))
                    FMT_LOG_ERROR("bg.battleground", "Isle of Conquest: There was an error spawning creature {}", agmar.Entry);
            }
            break;
        case GO_ALLIANCE_GATE_1:
        case GO_ALLIANCE_GATE_2:
        case GO_ALLIANCE_GATE_3:
            if (!GetBGCreature(BG_IC_NPC_HIGH_COMMANDER_HALFORD_WYRMBANE, false))
            {
                ...
                if (!AddCreature(halford.Entry, BG_IC_NPC_HIGH_COMMANDER_HALFORD_WYRMBANE, halford.Pos, halford.SpawnTime))
                    FMT_LOG_ERROR("bg.battleground", "Isle of Conquest: There was an error spawning creature {}", halford.Entry);
            }
            break;
        default:
            break;
