...
QuaternionData const shipParentRotation = { 0.f, 0.f, 1.f, -4.371139e-08f };

BattlegroundGOSpawnPoint const BG_SA_Ships[BG_SA_MAX_SHIPS_PER_TEAM * PVP_TEAMS_COUNT] =
{
...
uint32 const BG_SA_Factions[PVP_TEAMS_COUNT] =
{
    1732,
    1735,
};
...
bool BattlegroundSA::ResetObjs()
{
    ...
    for (uint32 i = BG_SA_CENTRAL_FLAG_A; i <= BG_SA_LEFT_FLAG_A; i += PVP_TEAMS_COUNT)
        SpawnBGObject(i + Defenders, RESPAWN_IMMEDIATELY);

    // spawn defender team portals
    for (uint32 i = BG_SA_PORTAL_DEFENDER_BLUE_A; i <= BG_SA_PORTAL_DEFENDER_RED_A; i += PVP_TEAMS_COUNT)
        SpawnBGObject(i + Defenders, RESPAWN_IMMEDIATELY);

    UpdateObjectInteractionFlags();

    // spawn bombs
    for (uint32 i = BG_SA_BOMB_A_1; i <= BG_SA_BOMB_A_91; i += PVP_TEAMS_COUNT)
    ...
    for (uint32 i = 0; i < BG_SA_MAX_GY; ++i)
    {
        ...
        sg = sDBCStoresMgr->GetWorldSafeLocsDBC(BG_SA_GYEntries[i]);
...
void BattlegroundSA::FillInitialWorldStates(WorldPackets::WorldState::InitWorldStates& packet)
{
    ...
    packet.Worldstates.emplace_back(BG_SA_TIMER_MINS, uint32(end_of_round / 60000));
    packet.Worldstates.emplace_back(BG_SA_TIMER_SEC_TENS, uint32((end_of_round % 60000) / 10000));
    packet.Worldstates.emplace_back(BG_SA_TIMER_SEC_DECS, uint32(((end_of_round % 60000) % 10000) / 1000));
