...
BattlegroundGOSpawnPoint const BattlegroundABDoors[PVP_TEAMS_COUNT] =
{
...
void BattlegroundAB::PostUpdateImpl(uint32 diff)
{
    if (GetStatus() == STATUS_IN_PROGRESS)
    {
        ...
        while (uint32 eventId = m_Events.ExecuteEvent())
        {
            switch (eventId)
            {
                ...
                case EVENT_NODE_CAPTURE_4:
                {
                    ...
                    {
                        SendBroadcastText(ABNodes[nodeId].TextAllianceTaken, CHAT_MSG_BG_SYSTEM_ALLIANCE);
                        PlaySoundToAll(BG_AB_SOUND_NODE_CAPTURED_ALLIANCE);
                    }
                    else
                    {
                        SendBroadcastText(ABNodes[nodeId].TextHordeTaken, CHAT_MSG_BG_SYSTEM_HORDE);
        ...
        uint32 capturedNodes[PVP_TEAMS_COUNT] = { 0, 0 };

        for (uint8 node = 0; node < BG_AB_DYNAMIC_NODES_COUNT; ++node)
        {
            for (uint8 teamIndex = TEAM_ALLIANCE; teamIndex < PVP_TEAMS_COUNT; ++teamIndex)
...
void BattlegroundAB::EventPlayerClickedOnFlag(Player* source, GameObject* /*target_obj*/)
{
    ...
    if (!(m_Nodes[node] == 0 || teamIndex == m_Nodes[node] % PVP_TEAMS_COUNT))
        return;
