...
void SmartAI::PausePath(uint32 delay, bool forced)
{
    ...
    {
        FMT_LOG_ERROR("scripts.ai.sai", "SmartAI::PausePath: Creature wanted to pause waypoint (current waypoint: {}) movement while already paused, ignoring. ({})", _currentWaypointNode, me->GetGUID().ToString());
        return;
    }
...
class SmartTrigger : public AreaTriggerScript
{
    public:
        ...
        bool OnTrigger(Player* player, AreaTriggerDBC const* trigger) override
        {
            ...
            FMT_LOG_DEBUG("scripts.ai", "AreaTrigger {} is using SmartTrigger script", trigger->ID);
            SmartScript script;
