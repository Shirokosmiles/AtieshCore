...
void LoadDisables()
{
    ...
    if (!result)
    {
        FMT_LOG_INFO("server.loading", ">> Loaded 0 disables. DB table `disables` is empty!");
        return;
    }
    ...
    do
    {
        ...
        {
            FMT_LOG_ERROR("sql.sql", "Invalid type {} specified in `disables` table, skipped.", type);
            continue;
        }
        ...
        switch (type)
        {
            case DISABLE_TYPE_SPELL:
                ...
                {
                    FMT_LOG_ERROR("sql.sql", "Spell entry {} from `disables` doesn't exist in dbc, skipped.", entry);
                    continue;
                }

                if (!flags || flags > MAX_SPELL_DISABLE_TYPE)
                {
                    FMT_LOG_ERROR("sql.sql", "Disable flags for spell {} are invalid, skipped.", entry);
                    continue;
                }
                ...
                if (flags & SPELL_DISABLE_MAP)
                {
                    for (std::string_view mapStr : Trinity::Tokenize(params_0, ',', true))
                    {
                        if (Optional<uint32> mapId = Trinity::StringTo<uint32>(mapStr))
                            ...
                            FMT_LOG_ERROR("sql.sql", "Disable map '{}' for spell {} is invalid, skipped.", std::string(mapStr), entry);
                    }
                }
                ...
                if (flags & SPELL_DISABLE_AREA)
                {
                    for (std::string_view areaStr : Trinity::Tokenize(params_1, ',', true))
                    {
                        if (Optional<uint32> areaId = Trinity::StringTo<uint32>(areaStr))
                            ...
                            FMT_LOG_ERROR("sql.sql", "Disable area '{}' for spell {} is invalid, skipped.", std::string(areaStr), entry);
                    }
                }
            ...
            case DISABLE_TYPE_LFG_MAP:
            {
                ...
                if (!mapEntry)
                {
                    FMT_LOG_ERROR("sql.sql", "Map entry {} from `disables` doesn't exist in dbc, skipped.", entry);
                    continue;
                }
                ...
                switch (mapEntry->InstanceType)
                {
                    ...
                    case MAP_ARENA:
                        FMT_LOG_ERROR("sql.sql", "Battleground map {} specified to be disabled in map case, skipped.", entry);
                        continue;
                }
                if (isFlagInvalid)
                {
                    FMT_LOG_ERROR("sql.sql", "Disable flags for map {} are invalid, skipped.", entry);
                    continue;
                }
                break;
            }
            case DISABLE_TYPE_BATTLEGROUND:
                ...
                {
                    FMT_LOG_ERROR("sql.sql", "Battleground entry {} from `disables` doesn't exist in dbc, skipped.", entry);
                    continue;
                }
                if (flags)
                    FMT_LOG_ERROR("sql.sql", "Disable flags specified for battleground {}, useless data.", entry);
                break;
            case DISABLE_TYPE_OUTDOORPVP:
                if (entry > MAX_OUTDOORPVP_TYPES)
                {
                    FMT_LOG_ERROR("sql.sql", "OutdoorPvPTypes value {} from `disables` is invalid, skipped.", entry);
                    continue;
                }
                if (flags)
                    FMT_LOG_ERROR("sql.sql", "Disable flags specified for outdoor PvP {}, useless data.", entry);
                break;
            case DISABLE_TYPE_ACHIEVEMENT_CRITERIA:
                if (!sAchievementMgr->GetAchievementCriteria(entry))
                {
                    FMT_LOG_ERROR("sql.sql", "Achievement Criteria entry {} from `disables` doesn't exist in dbc, skipped.", entry);
                    continue;
                }
                if (flags)
                    FMT_LOG_ERROR("sql.sql", "Disable flags specified for Achievement Criteria {}, useless data.", entry);
                break;
                ...
                switch (mapEntry->InstanceType)
                {
                    case MAP_COMMON:
                        if (flags & VMAP::VMAP_DISABLE_AREAFLAG)
                            FMT_LOG_INFO("misc", "Areaflag disabled for world map {}.", entry);
                        if (flags & VMAP::VMAP_DISABLE_LIQUIDSTATUS)
                            FMT_LOG_INFO("misc", "Liquid status disabled for world map {}.", entry);
                        break;
                    case MAP_INSTANCE:
                    case MAP_RAID:
                        if (flags & VMAP::VMAP_DISABLE_HEIGHT)
                            FMT_LOG_INFO("misc", "Height disabled for instance map {}.", entry);
                        if (flags & VMAP::VMAP_DISABLE_LOS)
                            FMT_LOG_INFO("misc", "LoS disabled for instance map {}.", entry);
                        break;
                    case MAP_BATTLEGROUND:
                        if (flags & VMAP::VMAP_DISABLE_HEIGHT)
                            FMT_LOG_INFO("misc", "Height disabled for battleground map {}.", entry);
                        if (flags & VMAP::VMAP_DISABLE_LOS)
                            FMT_LOG_INFO("misc", "LoS disabled for battleground map {}.", entry);
                        break;
                    case MAP_ARENA:
                        if (flags & VMAP::VMAP_DISABLE_HEIGHT)
                            FMT_LOG_INFO("misc", "Height disabled for arena map {}.", entry);
                        if (flags & VMAP::VMAP_DISABLE_LOS)
                            FMT_LOG_INFO("misc", "LoS disabled for arena map {}.", entry);
                        break;
                    default:
                        break;
                }
            ...
            case DISABLE_TYPE_MMAP:
            {
                ...
                {
                    case MAP_COMMON:
                        FMT_LOG_INFO("misc", "Pathfinding disabled for world map {}.", entry);
                        break;
                    case MAP_INSTANCE:
                    case MAP_RAID:
                        FMT_LOG_INFO("misc", "Pathfinding disabled for instance map {}.", entry);
                        break;
                    case MAP_BATTLEGROUND:
                        FMT_LOG_INFO("misc", "Pathfinding disabled for battleground map {}.", entry);
                        break;
                    case MAP_ARENA:
                        FMT_LOG_INFO("misc", "Pathfinding disabled for arena map {}.", entry);
                        break;
                    default:
                        break;
                }
    ...
    FMT_LOG_INFO("server.loading", ">> Loaded {} disables in {} ms", total_count, GetMSTimeDiffToNow(oldMSTime));
}
...
void CheckQuestDisables()
{
    ...
    if (!count)
    {
        FMT_LOG_INFO("server.loading", ">> Checked 0 quest disables.");
        return;
    }
    ...
    for (DisableTypeMap::iterator itr = m_DisableMap[DISABLE_TYPE_QUEST].begin(); itr != m_DisableMap[DISABLE_TYPE_QUEST].end();)
    {
        ...
        {
            FMT_LOG_ERROR("sql.sql", "Quest entry {} from `disables` doesn't exist, skipped.", entry);
            m_DisableMap[DISABLE_TYPE_QUEST].erase(itr++);
            continue;
        }
        if (itr->second.flags)
            FMT_LOG_ERROR("sql.sql", "Disable flags specified for quest {}, useless data.", entry);
        ++itr;
    }

    FMT_LOG_INFO("server.loading", ">> Checked {} quest disables in {} ms", count, GetMSTimeDiffToNow(oldMSTime));
}
