...
bool Corpse::Create(ObjectGuid::LowType guidlow, Player* owner)
{
    ...
    if (!IsPositionValid())
    {
        FMT_LOG_ERROR("entities.player", "Corpse (guidlow {}, owner {}) not created. Suggested coordinates isn't valid (X: {} Y: {})",
...
bool Corpse::LoadCorpseFromDB(ObjectGuid::LowType guid, Field* fields)
{
    ...
    if (!IsPositionValid())
    {
        FMT_LOG_ERROR("entities.player", "Corpse ({}, owner: {}) is not created, given coordinates are not valid (X: {}, Y: {}, Z: {})",
