...
void FormationMgr::AddCreatureToGroup(ObjectGuid::LowType leaderSpawnId, Creature* creature)
{
    ...
    if (itr != map->CreatureGroupHolder.end())
    {
        ...
        FMT_LOG_DEBUG("entities.unit", "Group found: {}, inserting creature {}, Group InstanceID {}", leaderSpawnId, creature->GetGUID().ToString(), creature->GetInstanceId());
    ...
    {
        //Create new group
        FMT_LOG_DEBUG("entities.unit", "Group not found: {}. Creating new group.", leaderSpawnId);
...
{
    FMT_LOG_DEBUG("entities.unit", "Deleting member pointer to GUID: {} from group {}", group->GetLeaderSpawnId(), member->GetSpawnId());
    ...
    if (group->IsEmpty())
    {
        ...
        FMT_LOG_DEBUG("entities.unit", "Deleting group with InstanceID {}", member->GetInstanceId());
...
void FormationMgr::LoadCreatureFormations()
{
    ...
    if (!result)
    {
        FMT_LOG_INFO("server.loading", ">>  Loaded 0 creatures in formations. DB table `creature_formations` is empty!");
        return;
    }
    ...
    do
    {
        ...
        // check data correctness
        {
            ...
            {
                FMT_LOG_ERROR("sql.sql", "creature_formations table leader guid {} incorrect (not exist)", member.LeaderSpawnId);
                continue;
            }

            if (!sObjectMgr->GetCreatureData(memberSpawnId))
            {
                FMT_LOG_ERROR("sql.sql", "creature_formations table member guid {} incorrect (not exist)", memberSpawnId);
                continue;
            }
    ...
    for (ObjectGuid::LowType leaderSpawnId : leaderSpawnIds)
    {
        ...
        {
            FMT_LOG_ERROR("sql.sql", "creature_formation contains leader spawn {} which is not included on its formation, removing", leaderSpawnId);
    ...
    FMT_LOG_INFO("server.loading", ">> Loaded {} creatures in formations in {} ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
{
    FMT_LOG_DEBUG("entities.unit", "CreatureGroup::AddMember: Adding unit {}.", member->GetGUID().ToString());

    //Check if it is a leader
    if (member->GetSpawnId() == _leaderSpawnId)
    {
        FMT_LOG_DEBUG("entities.unit", "Unit {} is formation leader. Adding group.", member->GetGUID().ToString());
        _leader = member;
    }
...
void CreatureGroup::FormationReset(bool /*dismiss*/)
{
    for (auto const& pair : _members)
    {
        if (pair.first != _leader && pair.first->IsAlive())
        {
            ...
            // FMT_LOG_DEBUG("entities.unit", "CreatureGroup::FormationReset: Set {} movement for member {}", dismiss ? "default" : "idle", pair.first->GetGUID().ToString());
        }
    }
