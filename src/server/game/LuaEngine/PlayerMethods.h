...
namespace LuaPlayer
{
    ...
    {
        ObjectGuid guid = Eluna::CHECKVAL<ObjectGuid>(L, 2);
        Eluna::Push(L, sMailMgr->GetMItem(guid.GetCounter()));
        return 1;
    }
    ...
    int SetReputation(lua_State* L, Player* player)
    {
        ...
        if (FactionDBC const* factionEntry = sDBCStoresMgr->GetFactionDBC(faction))
    ...
    {
        uint32 id = Eluna::CHECKVAL<uint32>(L, 2);
        CharTitlesDBC const* t = sDBCStoresMgr->GetCharTitlesDBC(id);
        if (t)
    ...
    {
        uint32 id = Eluna::CHECKVAL<uint32>(L, 2);
        AchievementDBC const* t = sDBCStoresMgr->GetAchievementDBC(id);
        if (t)
    ...
    int AdvanceAllSkills(lua_State* L, Player* player)
    {
        ...
        for (auto entry : sDBCStoresMgr->GetSkillLineDBCMap())
        {
            if (entry.second.CategoryID == SKILL_CATEGORY_LANGUAGES || entry.second.CategoryID == SKILL_CATEGORY_GENERIC)
                continue;

            if (player->HasSkill(entry.second.ID))
                player->UpdateSkill(entry.second.ID, step);
        }
        return 0;
    }
