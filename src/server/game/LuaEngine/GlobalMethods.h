...
namespace LuaGlobalFunctions
{
    ...
    int GetAreaName(lua_State* L)
    {
        ...
        AreaTableDBC const* areaEntry = sDBCStoresMgr->GetAreaTableDBC(areaOrZoneId);
        if (!areaEntry)
    ...
    int SendMail(lua_State* L)
    {
        ...
        if (!itemlist.empty())
        {
            ...
            sMailMgr->SendMailWithItemsByGUID(senderGUIDLow, receiverGUIDLow, MAIL_NORMAL, subject, text, money, itemlist,
                MAIL_CHECK_MASK_NONE, delay, cod);
            itemlist.clear();
        }
        else
            sMailMgr->SendMailByGUID(senderGUIDLow, receiverGUIDLow, MAIL_NORMAL, subject, text, money, MAIL_CHECK_MASK_NONE, delay, cod);
    ...
    int AddTaxiPath(lua_State* L)
    {
        ...
        std::vector<TaxiPathNodeDBC> nodes;
        ...
        if (!pathId)
            pathId = sDBCStoresMgr->GetTaxiPathNodesByPath().size();
        if (sDBCStoresMgr->GetTaxiPathNodesByPath().size() <= pathId)
            sDBCStoresMgr->ResizeTaxiPathNodesByPath(pathId + 1);
        ...
        for (auto entry : nodes)
        {
            ...
            sDBCStoresMgr->AddTaxiNodesDBC(nodeId++, nodeEntry);
            sDBCStoresMgr->AddTaxiPathNodesByPath(pathId, index++, entry);
        }

        if (startNode >= nodeId)
            return 1;

        sDBCStoresMgr->AddTaxiPathSetBySource(startNode, nodeId - 1, pathId, price);

        TaxiPathDBC pathEntry;
        pathEntry.FromTaxiNode = startNode;
        pathEntry.ToTaxiNode = nodeId - 1;
        pathEntry.Cost = price;
        pathEntry.ID = pathId;
        sDBCStoresMgr->AddTaxiPathDBC(pathId, pathEntry);
        Eluna::Push(L, pathId);
        return 1;
    }
