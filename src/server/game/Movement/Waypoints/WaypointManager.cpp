...
void WaypointMgr::Load()
{
    ...
    if (!result)
    {
        FMT_LOG_INFO("server.loading", ">> Loaded 0 waypoints. DB table `waypoint_data` is empty!");
        return;
    }
    ...
    do
    {
        ...
        {
            FMT_LOG_ERROR("sql.sql", "Waypoint {} in waypoint_data has invalid move_type, ignoring", waypoint.id);
            continue;
        }
    ...
    FMT_LOG_INFO("server.loading", ">> Loaded {} waypoints in {} ms", count, GetMSTimeDiffToNow(oldMSTime));
}
