...
void MovementPacketSender::SendHeightChangeToMover(Unit* unit, float newHeight)
{
    ...
    mover->SendDirectMessage(WorldPackets::Movement::MoveSetCollisionHeight(unit->GetPackGUID(), mCounter, newHeight).Write());
}
...
void MovementPacketSender::SendHeightChangeToObservers(Unit* unit, float newHeight)
{
    ...
    WorldPackets::Movement::MoveUpdateCollisionHeight moveUpdateCollisionHeight;
    moveUpdateCollisionHeight.Status = unit->GetMovementInfo();
    moveUpdateCollisionHeight.Height = newHeight;
    unit->SendMessageToSet(moveUpdateCollisionHeight.Write(), mover);
}
...
void MovementPacketSender::SendTeleportAckPacket(Unit* unit, MovementInfo const& movementInfo)
{
    ...
    WorldPackets::Movement::MoveTeleport moveTeleport(movementInfo);
    moveTeleport.SequenceIndex = mCounter;
    mover->SendDirectMessage(moveTeleport.Write());
}
...
void MovementPacketSender::SendTeleportPacket(Unit* unit, MovementInfo const& movementInfo)
{
    ...
    unit->SendMessageToSet(WorldPackets::Movement::MoveUpdateTeleport(movementInfo).Write(), unit->GetPlayerMovingMe());
}
...
void MovementPacketSender::SendSpeedChangeToMover(Unit* unit, UnitMoveType mtype, float newRate)
{
    ...
    mover->SendDirectMessage(WorldPackets::Movement::MoveSetSpeed(MoveTypeToOpcode[mtype][1], unit->GetPackGUID(), mCounter, newSpeedFlat).Write());
}
...
void MovementPacketSender::SendSpeedChangeToObservers(Unit* unit, UnitMoveType mtype, float newRate)
{
    ...
    WorldPackets::Movement::MoveUpdateSpeed moveUpdateSpeed(MoveTypeToOpcode[mtype][2]);
    moveUpdateSpeed.Status = unit->GetMovementInfo();
    moveUpdateSpeed.Speed = newRate;
    unit->SendMessageToSet(moveUpdateSpeed.Write(), mover);
}

void MovementPacketSender::SendSpeedChangeToAll(Unit* unit, UnitMoveType mtype, float newRate)
{
    unit->SendMessageToSet(WorldPackets::Movement::MoveSplineSetSpeed(MoveTypeToOpcode[mtype][0], unit->GetPackGUID(), newRate).Write(), true);
}
...
void MovementPacketSender::SendKnockBackToMover(Unit* unit, float vcos, float vsin, float speedXY, float speedZ)
{
    ...
    WorldPackets::Movement::MoveKnockBack moveKnockBack(unit->GetPackGUID(), mCounter);
...
void MovementPacketSender::SendKnockBackToObservers(Unit* unit, float vcos, float vsin, float speedXY, float speedZ)
{
    ...
    WorldPackets::Movement::MoveUpdateKnockBack moveUpdateKnockBack;
    moveUpdateKnockBack.Status = unit->GetMovementInfo();
    moveUpdateKnockBack.Direction = Position(vcos, vsin);
    moveUpdateKnockBack.Speeds.HorzSpeed = speedXY;
    moveUpdateKnockBack.Speeds.VertSpeed = speedZ;
    unit->SendMessageToSet(moveUpdateKnockBack.Write(), mover);
}
...
void MovementPacketSender::SendMovementFlagChangeToMover(Unit* unit, MovementFlags mFlag, bool apply)
{
    ...
    mover->SendDirectMessage(WorldPackets::Movement::MoveSetFlag(opcode, unit->GetPackGUID(), mCounter).Write());
}
...
void MovementPacketSender::SendMovementFlagChangeToObservers(Unit* unit, MovementFlags mFlag, bool apply)
{
    ...
    WorldPackets::Movement::MoveUpdate moveUpdate(opcode, 64);
    moveUpdate.Status = unit->GetMovementInfo();
    unit->SendMessageToSet(moveUpdate.Write(), false);
}
...
void MovementPacketSender::SendMovementFlagChangeToAll(Unit* unit, MovementFlags mFlag, bool apply)
{
    ...
    unit->SendMessageToSet(WorldPackets::Movement::MoveSplineSetFlag(opcode, unit->GetPackGUID()).Write(), true);
}
