...
void MoveSpline::init_spline(MoveSplineInitArgs const& args)
{
    ...
    {
        FMT_LOG_DEBUG("misc", "MoveSpline::init_spline: zero length spline, wrong input data?");
...
#define CHECK(exp) \
    ...
    {\
        if (unit)\
            FMT_LOG_ERROR("misc.movesplineinitargs", "MoveSplineInitArgs::Validate: expression '{}' failed for {}", #exp, unit->GetDebugInfo());\
        else\
            FMT_LOG_ERROR("misc.movesplineinitargs", "MoveSplineInitArgs::Validate: expression '{}' failed for cyclic spline continuation", #exp); \
        return false;\
    }
    CHECK(path.size() > 1);
...
bool MoveSplineInitArgs::_checkPathBounds() const
{
    if (!(flags & MoveSplineFlag::Mask_CatmullRom) && path.size() > 2)
    {
        ...
        for (uint32 i = 1; i < path.size()-1; ++i)
        {
            ...
            {
                FMT_LOG_ERROR("misc", "MoveSplineInitArgs::_checkPathBounds check failed");
                return false;
            }
        }
    }
