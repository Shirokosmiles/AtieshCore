...
void WardenWin::Init(WorldSession* session, SessionKey const& K)
{
    ...
    FMT_LOG_DEBUG("warden", "Server side warden for client {} initializing...", session->GetAccountId());
    FMT_LOG_DEBUG("warden", "C->S Key: {}", ByteArrayToHexStr(_inputKey));
    FMT_LOG_DEBUG("warden", "S->C Key: {}", ByteArrayToHexStr(_outputKey));
    FMT_LOG_DEBUG("warden", "  Seed: {}", ByteArrayToHexStr(_seed));
    FMT_LOG_DEBUG("warden", "Loading Module...");

    MakeModuleForClient();

    FMT_LOG_DEBUG("warden", "Module Key: {}", ByteArrayToHexStr(_module->Key));
    FMT_LOG_DEBUG("warden", "Module ID: {}", ByteArrayToHexStr(_module->Id));
    RequestModule();
}
...
{
    FMT_LOG_DEBUG("warden", "Initialize module");
...
{
    FMT_LOG_DEBUG("warden", "Request hash");
...
void WardenWin::HandleHashResult(ByteBuffer &buff)
{
    ...
    if (response != Module.ClientKeySeedHash)
    {
        ...
        FMT_LOG_WARN("warden", "{} failed hash reply. Action: {}", _session->GetPlayerInfo(), penalty);
        return;
    }

    FMT_LOG_DEBUG("warden", "Request hash reply: succeed");
...
{
    FMT_LOG_DEBUG("warden", "Request data from {} (account {}) - loaded: {}", _session->GetPlayerName(), _session->GetAccountId(), _session->GetPlayer() && !_session->PlayerLoading());
    ...
    for (WardenCheckCategory category : EnumUtils::Iterate<WardenCheckCategory>())
    {
        ...
        {
            FMT_LOG_DEBUG("warden", "Finished all {} checks, re-shuffling", EnumUtils::ToConstant(category));
            Trinity::Containers::RandomShuffle(checks);
            checksIt = checks.begin();
        }
    }
    ...
    {
        FMT_LOG_DEBUG("warden", "Finished building warden packet, size is %zu bytes", buff.size());
        FMT_LOG_DEBUG("warden", "Sent checks: {}", idstring());
    }
    else
    {
        FMT_LOG_WARN("warden", "Finished building warden packet, size is %zu bytes, but expected {} bytes!", buff.size(), expectedSize);
        FMT_LOG_WARN("warden", "Sent checks: {}", idstring());
    }
...
{
    FMT_LOG_DEBUG("warden", "Handle data");
    ...
    if (Length != (buff.size() - buff.rpos()))
    {
        buff.rfinish();
        char const* penalty = ApplyPenalty(nullptr);
        FMT_LOG_WARN("warden", "{} sends manipulated warden packet. Action: {}", _session->GetPlayerInfo(), penalty);
        return;
    }
    ...
    if (!IsValidCheckSum(Checksum, buff.contents() + buff.rpos(), Length))
    {
        buff.rfinish();
        char const* penalty = ApplyPenalty(nullptr);
        FMT_LOG_WARN("warden", "{} failed checksum. Action: {}", _session->GetPlayerInfo(), penalty);
        return;
    }
    ...
    // TIMING_CHECK
    {
        ...
        {
            char const* penalty = ApplyPenalty(nullptr);
            FMT_LOG_WARN("warden", "{} failed timing check. Action: {}", _session->GetPlayerInfo(), penalty);
            return;
        }
        ...
        FMT_LOG_DEBUG("warden", "Server tick count now:    {}", ticksNow);
        FMT_LOG_DEBUG("warden", "Server tick count at req: {}", _serverTicks);
        FMT_LOG_DEBUG("warden", "Client ticks in response: {}", newClientTicks);
        FMT_LOG_DEBUG("warden", "Round trip response time: {} ms", ourTicks - newClientTicks);
    }
    ...
    for (uint16 const id : _currentChecks)
    {
        ...
        switch (check.Type)
        {
            case MEM_CHECK:
            {
                ...
                if (Mem_Result != 0)
                {
                    FMT_LOG_DEBUG("warden", "RESULT MEM_CHECK not 0x00, CheckId {} account Id {}", id, _session->GetAccountId());
                    checkFailed = id;
                    continue;
                }
                ...
                {
                    FMT_LOG_DEBUG("warden", "RESULT MEM_CHECK fail CheckId {} account Id {}", id, _session->GetAccountId());
                    FMT_LOG_DEBUG("warden", "Expected: {}", ByteArrayToHexStr(expected));
                    FMT_LOG_DEBUG("warden", "Got:      {}", ByteArrayToHexStr(response));
                    checkFailed = id;
                    continue;
                }

                FMT_LOG_DEBUG("warden", "RESULT MEM_CHECK passed CheckId {} account Id {}", id, _session->GetAccountId());
                break;
            }
            case PAGE_CHECK_A:
            case PAGE_CHECK_B:
            ...
            case MODULE_CHECK:
            {
                ...
                {
                    FMT_LOG_DEBUG("warden", "RESULT {} fail, CheckId {} account Id {}", EnumUtils::ToConstant(check.Type), id, _session->GetAccountId());
                    checkFailed = id;
                    continue;
                }

                FMT_LOG_DEBUG("warden", "RESULT {} passed CheckId {} account Id {}", EnumUtils::ToConstant(check.Type), id, _session->GetAccountId());
                break;
            }
            case LUA_EVAL_CHECK:
            {
                ...
                FMT_LOG_DEBUG("warden", "LUA_EVAL_CHECK CheckId {} account Id {} got in-warden dummy response ({})", id, _session->GetAccountId(), result);
                break;
            }
            case MPQ_CHECK:
            {
                ...
                if (Mpq_Result != 0)
                {
                    FMT_LOG_DEBUG("warden", "RESULT MPQ_CHECK not 0x00 account id {}", _session->GetAccountId());
                    checkFailed = id;
                    continue;
                }
                ...
                {
                    FMT_LOG_DEBUG("warden", "RESULT MPQ_CHECK fail, CheckId {} account Id {}", id, _session->GetAccountId());
                    checkFailed = id;
                    continue;
                }

                FMT_LOG_DEBUG("warden", "RESULT MPQ_CHECK passed, CheckId {} account Id {}", id, _session->GetAccountId());
                break;
            }
    ...
    if (checkFailed > 0)
    {
        ...
        FMT_LOG_WARN("warden", "{} failed Warden check {} ({}). Action: {}", _session->GetPlayerInfo(), checkFailed, EnumUtils::ToConstant(check.Type), penalty);
    }
